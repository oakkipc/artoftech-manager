...
export default function ClientsPage() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('ยืนยันการลบ Client นี้? การลบลูกค้านี้จะทำให้โปรเจคและงบประมาณที่ผูกไว้ไม่มีข้อมูลลูกค้า')) return
        try {
    ...
    const filteredClients = clients.filter(c =>
        ...
        c.company_tax_id?.toLowerCase().includes(searchQuery.toLowerCase())
    )
                ...
                <div className="sticky top-0 z-20 bg-midnight-950/80 backdrop-blur-xl border-b border-white/[0.04] px-4 sm:px-8 py-4 text-white">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-midnight-500">Manage your customers and business clients</p>
                        </div>
                        <button onClick={() => { setEditingId(null); setShowModal(true) }}
                            className="flex items-center justify-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium rounded-lg transition-all shadow-lg shadow-emerald-600/20 active:scale-95">
                ...
                <div className="p-4 sm:p-8 space-y-6">
                    ...
                    <div className="relative max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-white/[0.04] border border-white/[0.06] rounded-lg text-white text-sm focus:outline-none focus:border-emerald-500/40"
                        />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredClients.map((client) => (
                            <div key={client.id} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 hover:border-emerald-500/30 transition-all group">
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <div className="flex gap-1">
                                        <button onClick={() => startEdit(client)} className="p-2 text-midnight-600 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                                            <Edit2 className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(client.id)} className="p-2 text-midnight-600 hover:text-red-400 hover:bg-red-500/5 rounded-lg transition-colors">
                                    ...
                                    <div className="space-y-2.5">
                                        {client.contact_person && (
                                            <div className="flex items-center gap-3 text-midnight-400 text-sm">
                                                <Briefcase className="w-4 h-4 opacity-50 text-midnight-600" />
                                                {client.contact_person}
                                            </div>
                                        )}
                                        {client.email && (
                                ...
                                <div className="mt-6 pt-6 border-t border-white/[0.04]">
                                    <button onClick={() => router.push(`/budget?clientId=${client.id}`)} className="w-full flex items-center justify-between text-xs font-bold text-midnight-500 hover:text-white transition-colors group/btn">
                                        VIEW REVENUE & PROJECTS
                    ...
                    {filteredClients.length === 0 && !loading && (
                        <div className="flex flex-col items-center justify-center py-20 bg-white/[0.01] border border-dashed border-white/10 rounded-3xl">
                            <Building2 className="w-12 h-12 text-midnight-800 mb-4" />
                            <p className="text-midnight-500 italic">No clients found.</p>
                        </div>
                    )}
                </div>
            </main>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2">
                                    <label className="block text-[10px] font-bold text-midnight-500 uppercase tracking-wider mb-1.5 ml-1">Client / Company Name *</label>
                                    <input
                                        type="text"
                                        required
                                        value={formData.name}
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-midnight-500 uppercase tracking-wider mb-1.5 ml-1">Tax ID / Company ID</label>
                                    <input
                                        type="text"
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-emerald-600/20 active:scale-[0.98] disabled:opacity-50 mt-4 text-sm"
                            >
                                {submitting ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : editingId ? 'Update Client' : 'Create Client'}
                            </button>
                        </form>
                    </div>
                </div>
